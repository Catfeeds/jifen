...
：<?php echo ($admin_account["black"]); ?></strong></td>
</tr>
<tr>
          ...
          <th>银行卡号</th>
              ...
              <tbody>
                <tr></tr>
                  ...
                  <td><a href="<?php echo U('Distribution/memberDetail',array('id'=>$list['memberid']));?>"><img src="<?php echo ($list["headimgurl"]); ?>" width="50"></a></td>
                  <td><?php echo ($list["name"]); ?></td>
                  <td><?php echo ($list["bankName"]); ?></td>
                  <td><?php echo ($list["bankNumber"]); ?></td>
				  <td><?php echo ($list["tele"]); ?></td>
				  <td><?php echo sprintf("%.2f",$list[money]/100);?></td>
				  <td><?php echo (date('Y-m-d H:i:s',$list["applytime"])); ?></td>
				  <td><?php if(($list["status"]) == "0"): ?>未审核<?php endif; if(($list["status"]) == "1"): ?>已审核<?php endif; if(($list["status"]) == "2"): ?>已打款<?php endif; ?></td>
                   <td class="norightborder">
                     <?php if(($list["status"]) == "0"): ?><a href="<?php echo U('Distribution/changeStatus',array('id'=>$list['id'],'status'=>1));?>" onclick="return confirm('确定通过审核么？');" style="color:blue" title="审核通过">审核通过</a>&nbsp;&nbsp;<?php endif; if(($list["status"]) == "1"): ?><a href="<?php echo U('Distribution/changeStatus',array('id'=>$list['id'],status=>2));?>" style="color:blue" onclick="return confirm('确定已经打款了么？');" title="打款完成">打款完成</a><?php endif; ?>
                   </td>
