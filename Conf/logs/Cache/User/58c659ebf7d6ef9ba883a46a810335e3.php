...
<div class="content" <?php if(session('isQcloud') == true): ?>style="float:center;"<?php endif; ?>>
          ...
          <form method="post"   action="<?php echo U('Article/set');?>"  enctype="multipart/form-data" >
            ...
            <TABLE class="userinfoArea" border="0" cellSpacing="0" cellPadding="0" width="100%">
              <TBODY>
               <TR>
                  ...
                  <TD><input type="text" class="px" id="title" value="<?php echo ($set["title"]); ?>"    name="title" style="width:580px;"> </TD>
                </TR>
               <!--  <TR>
                  <TH><span class="red">*</span><label for="title">分类：</label></TH>
                  <TD>
                    <select name="classid" id="classid">
                      <option value="0">--文章分类--</option>
                      <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><option value="<?php echo ($list["id"]); ?>" <?php if(($set["classid"]) == $list["id"]): ?>selected<?php endif; ?>><?php echo ($list["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                  </TD>
                </TR> -->
                 <TR>
                  <TH valign="top"><label for="url">文章外链网址：</label></TH>
                  <TD><input type="text" class="px" id="url" value="<?php echo ($set["url"]); ?>" name="url" style="width:280px;"><br /><span class="red">如需跳转到其他网址，请在输入框右侧选择或这里填写网址(例如：http://baidu.com，记住如果填写必须有http://)</span></TD>
                </TR>
                <TR>
                  <TH></TH>
