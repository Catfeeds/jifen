          ...
          <th>操作</th>
              ...
              <tbody>
                <tr></tr>
                  ...
                  <td><?php echo (($list["level"]["name"])?($list["level"]["name"]):"游客"); ?></td>
                  <!-- <td><?php echo ($list["creater"]); ?></td> -->
                  <td><?php echo ($list["bindac"]["nickname"]); ?></td>
                  <!-- <td><?php echo ($list["member"]["nickname"]); ?></td> -->
                  <td><?php echo (date('Y-m-d H:i:s',$list["addtime"])); ?></td>
                  <td class="norightborder">
                    <a href="<?php echo U('Distribution/account',array('id'=>$list['id'],'type'=>'detail','p'=>$_GET['p']));?>" title="详细">详细</a> | 
                    <!-- <a href="javascript:drop_confirm('您确定要删除吗?', '<?php echo U('Distribution/deleteAccount',array('id'=>$list['id']));?>');" title="删除">删除</a> | -->
                    <?php if(($list['admin'] != 1) and ($list['bartender'] != 1)): ?><a href="<?php echo U('Distribution/account',array('id'=>$list['id'],'type'=>'topup','p'=>$_GET['p']));?>" title="充值绿色咪豆">充值</a><?php endif; ?>
                  </td>
