...
<div class="content">
  ...
  <div class="msgWrap">
    <?php if(is_array($branch_list)): $i = 0; $__LIST__ = $branch_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><div class="branch_item">
        <div class="branch_item_con">
          <a href="<?php echo U('Distribution/topupRecord',array('type'=>'list','cbid'=>$list['id']));?>">
            <p><?php echo ($list["name"]); ?></p>
            <p><?php echo ($list["red"]); ?></p>
          </a>
        </div>
        <p class="branch_proportion">占比:<a data-cbid="<?php echo ($list["id"]); ?>" id="change_branch_proportion" href="javascript:;" contenteditable="true"><?php echo ($list["proportion"]); ?></a>%</p>
...
<script>
  $('#change_branch_proportion').bind('keyup',function(){
    ...
    if(!isNaN(p)){
      $.ajax({
        url:"<?php echo U('Distribution/changeBranchProportion');?>",
        data:{p:p,cbid:cbid},
        dataType:'json',
