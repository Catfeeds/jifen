...
<script language="javascript" type="text/javascript" src="<?php echo RES;?>/My97DatePicker/WdatePicker.js"></script>

<div class="content">

  <div class="cLineB">
    <h4 class="left" style="color: red;">代理点:<?php echo ($agent["name"]); ?>&nbsp;推广账号总数:<?php echo count($accounts);?></h4>
  </div>
  <div class="cLine">
    <div class="pageNavigator">
      <span>
        <form action="<?php echo U('Distribution/agent',array('type'=>'accountdetails','id'=>$agent['id']));?>" method="post" style="float: left;">
          关键词：
          <input type="text" name="name" class="px" value="<?php echo ($_REQUEST['name']); ?>" placeholder="输入账号名">
          <input class="Wdate" id="starttime" name="starttime" value="<?php echo ($_POST['starttime']); ?>" type="text" onClick="WdatePicker()" style="width:100px">&nbsp;到&nbsp;<input class="Wdate" id="endtime" name="endtime" value="<?php echo ($_POST['endtime']); ?>" type="text" onClick="WdatePicker()" style="width:100px">
  ...
  <div class="msgWrap">
    ...
    <table class="ListProduct" border="0" cellspacing="0" cellpadding="0" width="100%">
      ...
      <tbody>
        <tr></tr>
        <?php if(is_array($accounts)): $i = 0; $__LIST__ = $accounts;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><tr>
            <td><?php echo ($list["id"]); ?></td>
            <td><?php echo ($list["username"]); ?></td>
            <td style="color: green;"><?php echo (($list["green"])?($list["green"]):0); ?></td>
            <td style="color: red;"><?php echo (($list["red"])?($list["red"]):0); ?></td>
            <td><?php echo (($list["black"])?($list["black"]):0); ?></td>
            <td><?php echo ($list["bindaid"]); ?></td>
            <td><?php echo (date('Y-m-d',$list["addtime"])); ?></td>
            <td class="norightborder">
              <a href="<?php echo U('Distribution/agent',array('type'=>'loweraccount','id'=>$list['id']));?>">下级账号</a>
            </td>
