...
class WeixinAction extends Action
{
    ...
    private function reply($data)
    {
        ...
        if('subscribe' == $data['Event']||$data['Event'] == 'SCAN'){
            ...
            // $from_account = $account_db->where(array('mid'=>$my['id']))->find();
            ...
            if($sceneid){
                $from_member = $db->where(array('id'=>$sceneid))->find();
            }
            if(!$my){
                ...
                if($myid){
                    ...
                    if($from_member){
                        if($db->where(array('token'=>$this->token,'wecha_id'=>$this->data['FromUserName'],'id'=>array('neq',$myid)))->find()){
                            ...
                            $data_p = '{"touser":"'.$from_member['wecha_id'].'","msgtype":"news","news":{"articles":[{"title":"您有新朋友加入，赶紧看看吧","description":"亲：您有新朋友加入","url":"'.C('site_url').U('Wap/Distribution/memberList',array('type'=>'first')).'","picurl":""}]}}';
