define(['jquery','ajaxfileupload','velocity','velocity-ui'],function($,aimg){
	...
	PInfoEdit.prototype._EditClick=function(){
		...
		if(isAndroid){
			$('#info_edit_wrap').hide().animate({'left':0},300,function(){
				$('#info_edit_wrap').animate({'left':'100%'});
				$('#info_edit_item').focus();
			});
		}
		$(document).on('click','.editinfo_btn',function(){
			...
			if(name == 'password'){
				...
				$('.info_edit_old_password').hide();
				$('.info_edit_new_password').hide();
			}
			data = info_item.text();
	...
	PInfoEdit.prototype._EditClose=function(){
		$(document).on('click','#close_info_edit',function(){
		...
		$(document).on('click','#save_info',function(){
			...
			if (new_info) {
				...
				if(name == 'password'){
					...
					$.ajax({
						url: getAction('Agent','judgeOldPwd'),
						data: {
